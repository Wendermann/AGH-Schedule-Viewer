// Stan i logika strony planu: plany z katalogu dane/, tydzień typowy
// i kalendarzowy, kolizje z tygodniami, historia zmian, link „Udostępnij”.
// Ukrywanie, łączenie i kolizje liczy plan.js, a token stanu share.js.
// Widok (Alpine.js) jest w planview.js.
...
export function statusMessage(status, plan) {
  switch (status) {
    case "cache":
      return `Ostatnie odświeżenie planu ${plan.code} się nie udało, bo USOSweb nie odpowiadał. Skład planu pochodzi z kopii z ${stamp(plan.pageFetchedAt)}, a daty spotkań z ${stamp(plan.fetchedAt)}.`;
    case "empty":
      return `W cyklu ${plan.cycle} grupa przedmiotów ${plan.code} nie ma żadnych zajęć.`;
    default:
      return "";
  }
}
...
// Link do planu, który działa bez tokenu (np. ze strony startowej).
export const planHref = (base, code, cycle) =>
  `${base}plan.html?plan=${encodeURIComponent(code)}&cykl=${encodeURIComponent(cycle)}`;
...
export function createApp(data, { storageKey, loadPlan }) {
  ...
  const app = {
    ...
    // Plan z katalogu dane/, wczytany raz i trzymany w pamięci.
    async ensurePlan(code) {
      if (plansByCode.has(code)) return plansByCode.get(code);
    ...
    // Stan danych: kopia z cache albo plan bez zajęć.
    status() {
      const plans = app.activePlans();
      const stale = plans.find((p) => p.stale);
      if (stale) return { kind: "cache", plan: stale };
      if (plans.length && plans.every((p) => !p.activities.length)) return { kind: "empty", plan: plans[0] };
      return { kind: "ok", plan: plans[0] ?? null };
    },
    ...
    async addPlan(code) {
      if (app.isActive(code)) return;
      const plan = await app.ensurePlan(code);
  ...
  app.restore = async function restore() {
    ...
    if (!token && params.get("plan")) {
      ...
      try {
        await app.ensurePlan(code);
        app.state.plans = [{ kind: "g", code, cycle: plansByCode.get(code).cycle }];
        return null;
      } catch (error) {
    ...
    let state;
    try {
      state = await decodeState(token);
    } catch {
      return "Link „Udostępnij” jest uszkodzony albo pochodzi z nowszej wersji strony.";
    }
    const loaded = [];
    for (const ref of state.plans) {
      try {
        await app.ensurePlan(ref.code);
        loaded.push({ ...ref, cycle: plansByCode.get(ref.code).cycle });
      } catch (error) {
        problem = error.message;
      }
    }
    if (loaded.length) app.state = { ...state, plans: loaded, week: state.week ?? defaultWeek };
    return problem;
  };
...
export function describeChange(change) {
  ...
  if (fields.has("dates")) {
    const list = (dates) => dates.map((d) => `${shortDate(d)}${d.slice(10)}`).join(", ") || null;
    rows.push({ label: "daty", before: list(change.datesRemoved ?? []), after: list(change.datesAdded ?? []) });
  }
  const parts = [];
  ...
  if (fields.has("dates")) parts.push(change.datesAdded?.length ? "zmienione daty spotkań" : "odwołane spotkania");
